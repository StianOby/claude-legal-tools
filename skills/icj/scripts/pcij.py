...
The Court hosts the PCIJ collection at three pages:
  ...
  /pcij-series-b    Advisory opinions 1923-1930  (codes B01..B18)
  /pcij-series-ab   Judgments, Orders, Advisory opinions from 1931  (codes A/B 40..)

Each page renders its cases as an h3 (the case code, e.g. "A10") followed
by an h4 with the case name ('A10 "Lotus"') and one h6 per document
(judgment, dissenting opinions, annexes) containing the PDF link.
...
# Recognise codes like A01, A24, B04, "A/B 53", "A/B53", "AB 53".
_CODE_RE = re.compile(r"^(A/B|AB|A|B)\s*[-/]?\s*(\d{1,3})", re.IGNORECASE)
...
def _parse_series_page(html: str) -> list[dict]:
    ...
    # Walk H3 (case code), H4 (case name) and H6 (PDF links) in order.
    for el in main.iter():
        if el.tag == "h3":
            txt = el.get_text()
            ...
            if code is None:
                continue
            current = {"code": code, "raw_code": txt, "title": None, "documents": []}
            cases.append(current)
        elif el.tag == "h4" and current is not None:
            txt = el.get_text()
            # Strip leading code if echoed
            stripped = re.sub(r"^(?:A/B|AB|A|B)\s*\d{1,3}\s*", "", txt, count=1).strip()
            if stripped:
                current["title"] = stripped
        elif el.tag == "h6" and current is not None:
            a = el.find("a")
